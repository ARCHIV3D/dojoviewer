...
//    buttonDiv.className = "field";
    buttonDiv.innerHTML = `<img class="imgButton" src="icons/icon-close.png" srcset="icons2x/icon-close.png 2x" title="Fermer le menu"/>`;
...
function buildLinkMenuButton(label, link, icon = null, error = false) {
    ...
    var buttonDiv = document.createElement("td");
    buttonDiv.className = error ? "menu-button-error" : "menu-button-hum";
...
function buildErrorPopup(errors, warns, span = true) {
    ...
    if (errors && errors.length > 0) {
        buttonDiv.innerHTML = `<img src="icons/icon-error.png" srcset="icons2x/icon-error.png 2x" title="Erreur"/>`
    } else {
        buttonDiv.innerHTML = `<img src="icons/icon-warn.png" srcset="icons2x/icon-warn.png 2x" title="Avertissement"/>`
    }
...
function buildAddRoomButton(roomMetadata, room = null, errors = null) {
    ...
    tdenergy.innerHTML = `${roomMetadata.energy}<img src="icons/icon-energy.png" srcset="icons2x/icon-energy.png 2x" title="Énergie"/>`;
    ...
    tdcapacity.innerHTML = `${roomMetadata.capacity}<img src="icons/icon-capacity.png" srcset="icons2x/icon-capacity.png 2x" title="Capacité"/>`;
    roomButtonDiv.appendChild(tdcapacity);

    var tdresources = document.createElement("td")
    tdresources.className = "field clickable";
    tdresources.innerHTML = `<img onclick="showResources()" src="icons/icon-resources.png" srcset="icons2x/icon-resources.png 2x" title="Ressources"/>`;
...
function doShowErrors() {
    ...
    if (errorList && errorList.length > 0) {
        ...
        var menuDiv = buildMenu();
        menuDiv.appendChild(buildMenuHeaderLine("Pas d'erreurs trouvées", 3, "icon-ok"));
    }
...
function showResources() {
    ...
    menuDiv.appendChild(buildMenuHeaderLine((metadata ? ("Ressources pour " + metadata.name) : "Ressources globales"), 4, "icon-resources"));
...
function doPngMenu() {
    ...
    menuDiv.appendChild(buildMenuInput("Échelle", scaleInput, "pixels/meter"));

    var sizeDisplay = document.createElement("div");
    sizeDisplay.id = "png-size";
    sizeDisplay.class = "field";
    menuDiv.appendChild(buildMenuInput("Taille", sizeDisplay, "pixels"));

    menuDiv.appendChild(buildMenuButton("Créer l'image", doPngLinkMenu));
...
function doPngLinkMenu() {
    ...
    var menuDiv = buildMenu();
    menuDiv.appendChild(buildMenuHeaderLine("Télécharger", 3));
...
function updateErrorIcon(element) {
    if (element.errorList && element.errorList.length > 0) {
        element.innerHTML = `<img src="icons/icon-error.png" srcset="icons2x/icon-error.png 2x" title="Erreurs"/>`;

    } else if (element.warnList && element.warnList.length > 0) {
        element.innerHTML = `<img src="icons/icon-warn.png" srcset="icons2x/icon-warn.png 2x" title="Avertissements"/>`;
